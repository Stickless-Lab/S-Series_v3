...
I highly recommend downloading the Fusion assembly file [here](cad/S-Series_v3.4.78.f3z) and opening it in Fusion, which is free to use. This file shows the full assembly, so you can clearly see where every part needs to go.
...
You will also need a screw kit with screws ranging from M3 to M5, and lengths from around 6 mm to 40 mm. I do not have an exact screw count for this project, but a kit similar to [this one](https://www.amazon.com.be/-/nl/KOXXIAMOQ-verzonken-schroeven-moeren-sluitringen/dp/B0CJ6LCV9W/ref=sr_1_3?sr=8-3) should be good. I apologize for that and the next project I'll make will have that. For screws that might come lose use the Hex Nylon Insert Nuts you bought.

## 3D print the parts

Next, 3D print all the required parts.

Print the parts in [PLA-Parts](cad/PLA-Parts) using PLA.

Print the parts in [PETG-Parts](cad/PETG-Parts) using PETG.

When you open the `.f3z` file mentioned earlier, the PLA parts are shown in blue and the PETG parts are shown in red.
...
Be careful when screwing parts onto the `BatteryHolder`. You do not want to puncture the battery. This is especially important for parts like:

* `SuspensionHolder-Left`
* `SuspensionHolder-Right`
* `PF-ReceiverHolder`
* `PB-ReceiverHolder`
* ...

I recommend screwing these parts into the `BatteryHolder` first, and only putting the battery inside after that.

In the Fusion assembly, the white parts are the bought components.
...
[firmware/S-Series-v3.cpp](https://github.com/Stickless-Lab/S-Series_v3/blob/main/firmware/S-Series-v3.cpp)

More detailed firmware upload instructions are available here:

[firmware/README.md](https://github.com/Stickless-Lab/S-Series_v3/blob/main/firmware/README.md)
...
[pcb](https://github.com/Stickless-Lab/S-Series_v3/tree/main/pcb)
